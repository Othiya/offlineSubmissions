...
//            printData(accidentCount, "statistics/accident.csv");
    ...
    private void formGroup() {
        ...
        for (Pedestrian pedestrian : pedestrians) {
            ...
            for (Vector<Pedestrian> group : groups) {
                for (Pedestrian otherPedestrian : group) {
                    // Check if the pedestrian is within the specified radius of any pedestrian in the group
                    if (distance(pedestrian, otherPedestrian) <= stripWidth*30 && pedestrian.getDirection()==otherPedestrian.getDirection()) {
        ...
        for (Vector<Pedestrian> group : groups) {
            ...
            for (Pedestrian ped : group) {
                ped.setInitPos(init + (ped.getInitPos() - init)*stripWidth*5 / group.size());
            }
        }
    }
