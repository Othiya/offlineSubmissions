...
#S_TYPE = ['sll', 'srl']
I_TYPE = ['addi', 'subi', 'andi', 'ori', 'beq', 'bneq', 'lw', 'sw','sll', 'srl']
J_TYPE = ['j']
...
def generate_control_rom():
    ...
    for i in range(len(sequence)):
        ...
        elif serial[sequence[i]] in I_TYPE[8:]: # S-Type
...
def change(line_no, opcode, *args):
    ...
    elif opcode in R_TYPE[8:]: # S-Type
        bin_instruction += registers[args[1]] # source register
        bin_instruction += registers[args[0]] # destination register
        bin_instruction += bin(int(args[2]))[2:].zfill(8) # shift amount
    ...
    # lw $t0, 1($t1) : Opcode SrcReg1($t1) DstReg($t0) signed-int       | *($t1+1) = $t0
    elif opcode in I_TYPE[:8]: # I-Type
        ...
        bin_instruction += bin(immediate)[2:].zfill(8) # immediate value / address
