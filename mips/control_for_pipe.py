...
def generate_control_rom_row(reg_write,alu_op):
    return hex(int(reg_write+alu_op, 2))[2:].zfill(3)
...
def generate_control_rom():
    ...
    for i in range(len(sequence)):
        if serial[sequence[i]] in R_TYPE[:5]: # Register Only
            print(generate_control_rom_row('1', ALU_OP[serial[sequence[i]]]), end=' ')
        elif serial[sequence[i]] in R_TYPE[5:]: # Shift
            print(generate_control_rom_row('1',ALU_OP[serial[sequence[i]]]), end=' ')
        elif serial[sequence[i]] in I_TYPE[:4]: # immediate
            print(generate_control_rom_row( '1',ALU_OP[serial[sequence[i]][:-1]]), end=' ')
        elif serial[sequence[i]] in I_TYPE[4:6]: # Branch
            is_beq = ('1', '0') if serial[sequence[i]] == 'beq' else ('0', '1')
            print(generate_control_rom_row('0',ALU_OP['sub']), end=' ')
        elif serial[sequence[i]] in I_TYPE[6:8]: # Memory
            if serial[sequence[i]] == 'sw':
                print(generate_control_rom_row('0',ALU_OP['add']), end=' ')
            else:
                print(generate_control_rom_row('1',ALU_OP['add']), end=' ')
