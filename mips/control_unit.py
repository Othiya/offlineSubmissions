...
serial = {'A': 'add', 'B': 'addi', 'C': 'sub', 'D': 'subi', 'E': 'and', 'F': 'andi', 'G': 'or', 'H': 'ori', 'I': 'sll', 'J': 'srl', 'K': 'nor', 'L': 'sw', 'M': 'lw', 'N': 'beq', 'O': 'bneq', 'P': 'j'}

sequence = 'BDFJMHEOIKPCGNAL'
instruction = {}
...
R_TYPE = ['add', 'sub', 'and', 'or', 'nor','sll', 'srl']
S_TYPE = ['sll', 'srl']
I_TYPE = ['addi', 'subi', 'andi', 'ori', 'beq', 'bneq', 'lw', 'sw']
J_TYPE = ['j']

ALU_OP = {'add': '000', 'sub': '001', 'or': '010', 'and': '011', 'nor': '100', 'sll': '101', 'srl': '110'}
...
def generate_control_rom_row(reg_dst, alu_src, mem_to_reg, reg_write, mem_read, mem_write, branch, branch_neq, jump,shift, alu_op):
    return hex(int(reg_dst + alu_src + mem_to_reg + reg_write + mem_read + mem_write + branch + branch_neq + jump +shift + alu_op, 2))[2:].zfill(3)
...
def generate_control_rom():
    ...
    print('\t', end='')
    for i in range(len(sequence)):
        if serial[sequence[i]] in R_TYPE[:5]: # Register Only
            print(generate_control_rom_row(REG_DST_0_3, ALU_SRC_REG, MEM_TO_REG_ALU_RESULT, '1', '0', '0', '0', '0', '0','0', ALU_OP[serial[sequence[i]]]), end=' ')
        elif serial[sequence[i]] in R_TYPE[5:]: # Shift
            print(generate_control_rom_row(REG_DST_4_7, ALU_SRC_INST, MEM_TO_REG_ALU_RESULT, '1', '0', '0', '0', '0', '0','1', ALU_OP[serial[sequence[i]]]), end=' ')
        elif serial[sequence[i]] in I_TYPE[:4]: # immediate
            print(generate_control_rom_row(REG_DST_4_7, ALU_SRC_INST, MEM_TO_REG_ALU_RESULT, '1', '0', '0', '0', '0', '0','0', ALU_OP[serial[sequence[i]][:-1]]), end=' ')
        elif serial[sequence[i]] in I_TYPE[4:6]: # Branch
            is_beq = ('1', '0') if serial[sequence[i]] == 'beq' else ('0', '1')
            print(generate_control_rom_row(REG_DST_4_7, ALU_SRC_REG, MEM_TO_REG_ALU_RESULT, '0', '0', '0', is_beq[0], is_beq[1], '0','0', ALU_OP['sub']), end=' ')
        elif serial[sequence[i]] in I_TYPE[6:8]: # Memory
            if serial[sequence[i]] == 'sw':
                print(generate_control_rom_row(REG_DST_4_7, ALU_SRC_INST, MEM_TO_REG_MEM_RESULT, '0', '0', '1', '0', '0', '0','0', ALU_OP['add']), end=' ')
            else:
                print(generate_control_rom_row(REG_DST_4_7, ALU_SRC_INST, MEM_TO_REG_MEM_RESULT, '1', '1', '0', '0', '0', '0','0', ALU_OP['add']), end=' ')
        elif serial[sequence[i]] in J_TYPE: # Jump
            print(generate_control_rom_row(REG_DST_4_7, ALU_SRC_REG, MEM_TO_REG_ALU_RESULT, '0', '0', '0', '0', '0', '1','0', ALU_OP['add']), end=' ')
    print()
